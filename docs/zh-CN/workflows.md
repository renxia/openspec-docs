...
本指南涵盖 OpenSpec 的常见工作流模式及其适用场景。基础设置请参阅[快速入门](getting-started.md)。命令参考请查阅[命令](commands.md)。
...
批量归档会检测多个变更是否涉及相同规范，并通过检查实际实现来解决冲突。
...
如果规范未同步，归档会提示。它不会阻止任务未完成的情况，但会发出警告。
