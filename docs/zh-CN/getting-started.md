...
本指南说明在安装和初始化 OpenSpec 后，它的工作方式。有关安装说明，请参阅[主 README](index.md#quick-start)。
...
默认的全局配置文件是 `core`，包含 `propose`、`explore`、`apply` 和 `archive`。您可以通过 `openspec config profile` 然后 `openspec update` 来启用扩展的工作流命令。
...
- **`specs/`** - 事实来源。这些规格描述了您系统当前的行为。按领域组织（例如，`specs/auth/`、`specs/payments/`）。

- **`changes/`** - 提议的修改。每个变更都有自己的文件夹，包含所有相关工件。当变更完成时，其规格会合并到主 `specs/` 目录中。
...
| 工件 | 用途 |
|----------|---------|
| `proposal.md` | "为什么"和"什么" - 记录意图、范围和方法 |
...
增量规格是 OpenSpec 中的关键概念。它们显示相对于当前规格正在发生的变化。
...
变更文件夹会移动到 `openspec/changes/archive/` 以供审计历史记录。

## 示例：您的第一个变更

让我们逐步介绍如何向应用程序添加深色模式。
...
如果您启用了扩展的工作流配置文件，您也可以分两步完成：`/opsx:new` 然后 `/opsx:ff`（或 `/opsx:continue` 增量进行）。
