...
Ви можете увімкнути розширені робочі процеси (`new`, `continue`, `ff`, `verify`, `sync`, `bulk-archive`, `onboard`) через `openspec config profile`, а потім запустити `openspec update`.
...
\* Команди Codex встановлюються в глобальному домашньому каталозі Codex (`$CODEX_HOME/prompts/`, якщо встановлено, інакше `~/.codex/prompts/`), а не у каталозі вашого проєкту.

\*\* Файли підказок GitHub Copilot розпізнаються як користувацькі слеш-команди в розширеннях IDE (VS Code, JetBrains, Visual Studio). Copilot CLI наразі не використовує файли `.github/prompts/*.prompt.md` напряму.

## Неінтерактивне налаштування

Для CI/CD або скриптового налаштування використовуйте `--tools` (і опціонально `--profile`):
...
**Доступні ID інструментів (`--tools`):** `amazon-q`, `antigravity`, `auggie`, `bob`, `claude`, `cline`, `codex`, `codebuddy`, `continue`, `costrict`, `crush`, `cursor`, `factory`, `forgecode`, `gemini`, `github-copilot`, `iflow`, `junie`, `kilocode`, `kiro`, `opencode`, `pi`, `qoder`, `qwen`, `roocode`, `trae`, `windsurf`
...
- **Профіль Core (за замовчуванням):** `propose`, `explore`, `apply`, `archive`
- **Користувацький вибір:** будь-який підмножина всіх ID робочих процесів:
...
При виборі через конфігурацію профілю/робочого процесу OpenSpec генерує ці навички:
...
- [Початок роботи](getting-started.md) — Первинне налаштування